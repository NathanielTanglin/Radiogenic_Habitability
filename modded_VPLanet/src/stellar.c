/**
 * Modified by Nathaniel Tanglin
 * 
 * 11/16/25:
 *  Added SurfMagField option in InitializeOptionsStellar
 * 
 * 
 */
...
void ReadSurfMagField(BODY *body, CONTROL *control, FILES *files, OPTIONS *options,
                   SYSTEM *system, int iFile) {
   ...
   if (lTmp >= 0) {
    ...
    if (dTmp < 0) {
      body[iFile - 1].dSurfMagField = dTmp*1000; // If negative, work in Gauss
    } else {
      body[iFile - 1].dSurfMagField = dTmp; // If positive, work in Tesla (SI)
    }
    
    UpdateFoundOption(&files->Infile[iFile], options, lTmp, iFile);
  } else {
    body[iFile - 1].dSurfMagField = options->dDefault;
  }
}
...
void InitializeOptionsStellar(OPTIONS *options, fnReadOption fnRead[]) {
  ...
  fvFormattedString(&options[OPT_SURFMAGFIELD].cName,"dSurfMagField");
  fvFormattedString(&options[OPT_SURFMAGFIELD].cDescr,"Surface magnetic field strength of the host star");
  fvFormattedString(&options[OPT_SURFMAGFIELD].cDefault,"0.0001 Tesla");
  options[OPT_SURFMAGFIELD].dDefault = 0.0001;
  fvFormattedString(&options[OPT_SURFMAGFIELD].cDimension, "Tesla");
  options[OPT_SURFMAGFIELD].iType = 2;
  options[OPT_SURFMAGFIELD].bNeg = 1;
  fvFormattedString(&options[OPT_SURFMAGFIELD].cNeg, "Solar Field Strength");
  options[OPT_SURFMAGFIELD].dNeg = 1;
  options[OPT_SURFMAGFIELD].iFileType = 1;
  options[OPT_SURFMAGFIELD].iModuleBit = STELLAR;
  fnRead[OPT_SURFMAGFIELD] = &ReadSurfMagField;
...
void WriteSurfMagField(BODY *body, CONTROL *control, OUTPUT *output, SYSTEM *system,
                   UNITS *units, UPDATE *update, int iBody, double *dTmp,
                   char **cUnit) {

  *dTmp = body[iBody].dSurfMagField;
...
void InitializeOutputStellar(OUTPUT *output, fnWriteOutput fnWrite[]) {
  ...
  fvFormattedString(&output[OUT_SURFMAGFIELD].cName, "DSurfMagField");
  fvFormattedString(&output[OUT_SURFMAGFIELD].cDescr, "Stellar surface magnetic field");
  fvFormattedString(&output[OUT_SURFMAGFIELD].cNeg, "Gauss");
  output[OUT_SURFMAGFIELD].bNeg = 1;
  output[OUT_SURFMAGFIELD].dNeg = 1000;
  output[OUT_SURFMAGFIELD].iNum = 1;
  output[OUT_SURFMAGFIELD].iModuleBit = STELLAR;
  fnWrite[OUT_SURFMAGFIELD] = &WriteSurfMagField;
}
