...
# read data
data = np.loadtxt(path / "Solarsystem.Earth.forward")
...
ax3.plot(
    time * 10 ** -6, M_water_mo - M_water_atm / TO, label="Magma ocean", color=cmap(120)
)
...
# Save the figure
ext = get_args().ext
fig.savefig(path / f"Earth_5TO_Hamano.{ext}", bbox_inches="tight", dpi=300)
