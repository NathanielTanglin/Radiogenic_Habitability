...
# Run vplanet
vplanet.run(path / "vpl.in", quiet=True)

# read data
data = np.loadtxt(path / "GJ1132.GJ1132b.forward")
...
fig.suptitle(
    "GJ1132b: Initial water content " + str(M_water_mo[0]) + " TO + grey atmosphere",
    fontsize=16,
    fontweight="bold",
)
...
ax3.plot(
    time * 10 ** -6, M_water_mo - M_water_atm / TO, color=cmap(220), label="magma ocean"
)
...
# Save the figure
ext = get_args().ext
fig.savefig(path / f"Results_GJ1132b.{ext}", bbox_inches="tight", dpi=300)
